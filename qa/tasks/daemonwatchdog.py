...
class DaemonWatchdog(Greenlet):
    ...
    def bark(self):
        ...
        daemons = []
        daemons.extend(filter(not daemon.finished(), self.ctx.daemons.iter_daemons_of_role('osd', cluster=self.cluster)))
        daemons.extend(filter(not daemon.finished(), self.ctx.daemons.iter_daemons_of_role('mds', cluster=self.cluster)))
        daemons.extend(filter(not daemon.finished(), self.ctx.daemons.iter_daemons_of_role('mon', cluster=self.cluster)))
        daemons.extend(filter(not daemon.finished(), self.ctx.daemons.iter_daemons_of_role('rgw', cluster=self.cluster)))
        daemons.extend(filter(not daemon.finished(), self.ctx.daemons.iter_daemons_of_role('mgr', cluster=self.cluster)))
    ...
    def watch(self):
        ...
        while not self.stopping.is_set():
            ...
            daemon_failures = []
            daemon_failures.extend(filter(daemon.finished(), osds))
            daemon_failures.extend(filter(daemon.finished(), mons))
            daemon_failures.extend(filter(daemon.finished(), mdss))
            daemon_failures.extend(filter(daemon.finished(), rgws))
            daemon_failures.extend(filter(daemon.finished(), mgrs))
