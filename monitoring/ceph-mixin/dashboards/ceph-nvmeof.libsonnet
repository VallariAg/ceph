...
(import 'utils.libsonnet') {
  'ceph-nvmeof.json': $.dashboardSchema(
    ...
    .addTarget(
      ...
      .addTarget(
        $.addTargetSchema(
          expr="count(ceph_nvmeof_gateway_status==1 or ceph_nvmeof_gateway_status==2)",
          format='time_series',
          instant=true,
      ...
      .addTarget(
        $.addTargetSchema(
          expr="count(ceph_nvmeof_gateway_status==0 or ceph_nvmeof_gateway_status==-1 or ceph_nvmeof_gateway_status == -2)",
          format='time_series',
          instant=true,
          legendFormat="Down",
          range=false,
      ...
      .addTarget(
        $.addTargetSchema(
          expr="group by (ceph_daemon) (ceph_nvmeof_gateway_status == 0)",
          format='',
          instant=false,
          legendFormat="stopped - {{ ceph_daemon }} ",
          range=true,
          datasource='$datasource',
        )
      )
      .addTarget(
        $.addTargetSchema(
          expr="group by (ceph_daemon) (ceph_nvmeof_gateway_status == -1)",
          format='',
          instant=false,
          legendFormat="error - {{ ceph_daemon }}",
          range=true,
          datasource='$datasource',
        )
      )
      .addTarget(
        $.addTargetSchema(
          expr="group by (ceph_daemon) (ceph_nvmeof_gateway_status == -2)",
          format='',
          instant=false,
          legendFormat="unknown_state - {{ ceph_daemon }}",
          range=true,
          datasource='$datasource',
        )
      )
