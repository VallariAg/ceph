...
NVMEOF_GATEWAY_STATUS = ('ceph_daemon', 'hostname', 'version', 'status')
...
class Module(MgrModule, OrchestratorClientMixin):
    ...
    def _setup_static_metrics(self) -> Dict[str, Metric]:
        ...
        metrics['nvmeof_gateway_status'] = Metric(
            'untyped',
            'nvmeof_gateway_status',
            'NVMeoF Gateway Status',
            NVMEOF_GATEWAY_STATUS
        )
    ...
    def get_metadata_and_osd_status(self) -> None:
        ...
        if self.orch_is_available():
            nvmeof_daemons = raise_if_exception(self.list_daemons(daemon_type='nvmeof'))
            for daemon in nvmeof_daemons:
                value = int(daemon.status.value if daemon.status is not None else -10)
                self.metrics['nvmeof_gateway_status'].set(
                    value, (f"{daemon.daemon_type}.{daemon.daemon_id}",
                        str(daemon.hostname),
                        str(daemon.version),
                        ))
