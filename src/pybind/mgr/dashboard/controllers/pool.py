        ...
        if application_metadata is not None:
            def set_app(app_metadata, set_app_what):
                for app in app_metadata:
                    CephService.send_command('mon', 'osd pool application ' + set_app_what, pool=pool, app=app,
                                             yes_i_really_mean_it=True)

            original_app_metadata = set(
                cast(Iterable[Any], current_pool.get('application_metadata'))) if update_existing else set()
