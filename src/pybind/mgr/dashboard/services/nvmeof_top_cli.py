...
def get_lbg_gws_map(service_name: str): 
    ...
    if not pool_group:
        logger.error("Error getting pool namd and group name of the service")
        return {}
    pool, group = pool_group
    try:
        cmd = {
            'prefix': f'nvme-gw show {pool} {group}',
            # 'url': prometheus_url
        }
        ...
        if ret_status == 0 and out is not None:
            ...
            for gw in gws_info["Created Gateways:"]:
                gw_id = str(gw["gw-id"]).removeprefix("client.nvmeof.") # TODO: remove client from beginning of string
...
class NvmeofTopCollector:
    ...
    def get_sorted_namespaces(self, sort_pos: int, reverse_sort: bool):
        ...
        for ns in self.namespaces[self.subsystem_nqn]:
            ...
            if self.cmd_handler.args.get('server_addr'):
                lbg = ns.load_balancing_group
                daemon_name = self.lbg_gw[lbg] # TODO: init self.lbg_gw
    ...
    async def collect_io_data(self):
        ...
        if not self.cmd_handler.args.get('server_addr'):
            ...
            for gw in gw_conf["gateways"][service_name]:
                client = NVMeoFClient(group, gw["service_url"])
                # self.lbg_gw[client.gateway_addr] = self._get_gateway(client)
