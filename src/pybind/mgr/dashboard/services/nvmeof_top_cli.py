...
class NVMeoFTop:
    ...
    def to_stdout(self):
        ...
        if not self.args.get('no_headings'):
            sort_order = 'v' if self.reverse_sort else '^' 
            headers = NVMeoFTop.text_headers
            headers[sort_pos] = f'{headers[sort_pos]} {sort_order}'
            rows.append(NVMeoFTop.text_template.format(headers))
        if ns_data:
            for ns in ns_data:
