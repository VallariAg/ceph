...
class NVMeoFTop:
    ...
    reactors_headers = ['Thread', 'Busy', 'Idle']
    reactors_template = "{:<40}   {:>7}   {:>6}   {:>6}   {:>7}   {:>8}   {:>6}   {:>6}   {:>7}   {:>8}   {:^5}   {:>3}\n"
    # text_headers = ['']
...
class DataCollector:
    ...
    def get_reactor_data(self): # TODO
        thread_data = self._get_threads_stats()
        logger.info(f"VALLARI_DEBUG cephadm _get_threads_stats: {thread_data}")
        logger.info(f"VALLARI_DEBUG cephadm _get_threads_stats.threads: {thread_data.get('threads')}")
        tick_rate = thread_data.get('tick_rate')
